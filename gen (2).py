...
def generate_prime_private_keys(start, end, target_address):
        ...
        if is_probable_prime(value):
            ...
            if address == target_address:
                with open("win.txt", "w") as file:
                    file.write(f"Adresă: {address}\nCheie Privată: {private_key_hex}\n")
                print(f"Adresă și cheie privată salvate: {address}")
                break
            yield private_key_hex, address

# Exemplu de utilizare
start_value = 36893488147419103232
end_value = 55340232221128654848  # Valoare de sfârșit pentru exemplu
target_address = "13zb1hQbWVsc2S7ZTZnP2G4undNNpdh5so"

for private_key, address in generate_prime_private_keys(start_value, end_value, target_address):
