...
def main():
    ...
    # Lese die Seeds aus der TXT-Datei
    with open("seeds.txt", "r", encoding="ISO-8859-1") as seed_file:
        seeds = seed_file.readlines()

    for seed in seeds:
        seed = seed.strip()  # Entferne Leerzeichen und Zeilenumbrüche
        if not seed:
            continue  # Ignoriere leere Zeilen

        try:
            print(Fore.MAGENTA + f"Aktueller Seed: {seed}" + Fore.RESET)

            # Bitcoin Wallet
            hd_wallet_btc = HDWallet(symbol=BTC)
            hd_wallet_btc.from_mnemonic(mnemonic=seed)
            ...
            hd_wallet_eth.from_mnemonic(mnemonic=seed)
            ...
            if btc_balance is not None and btc_balance > 0:
                save_balance_and_seed(seed, btc_address, btc_balance)
            if eth_balance is not None and eth_balance > 0:
                save_balance_and_seed(seed, eth_address, eth_balance)
